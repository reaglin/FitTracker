...
import com.fittracker.databinding.ItemExerciseBinding

class ExerciseListAdapter(
    private val onEdit: (Exercise) -> Unit,
    private val onDelete: (Exercise) -> Unit
) : ListAdapter<Exercise, ExerciseListAdapter.ViewHolder>(DiffCallback()) {
    ...
    inner class ViewHolder(private val b: ItemExerciseBinding) :
        RecyclerView.ViewHolder(b.root) {

        fun bind(exercise: Exercise) {
            b.textName.text = exercise.name
            b.textTarget.text = "${exercise.quantity.display()} ${exercise.unit}"
            b.btnEdit.setOnClickListener { onEdit(exercise) }
            b.btnDelete.setOnClickListener { onDelete(exercise) }
        }
    }
